@extends('layouts.dashboard')

@section('title', 'Editar Noticia — Panel')
@section('page-title', 'Editar noticia')
@section('page-subtitle', Str::limit($noticia->titulo, 40))

@section('content')
<div class="row justify-content-center">
    <div class="col-lg-8">

        <a href="{{ route('dashboard.noticias.index') }}"
           class="btn btn-sm btn-outline-secondary mb-4">
            <i class="bi bi-arrow-left me-1"></i>Volver
        </a>

        {{-- Acciones rápidas --}}
        <div class="d-flex gap-2 mb-4">
            @if($noticia->estado === 'borrador')
                <form method="POST"
                      action="{{ route('dashboard.noticias.publicar', $noticia->id) }}">
                    @csrf
                    <button type="submit" class="btn btn-success btn-sm">
                        <i class="bi bi-send me-1"></i>Publicar ahora
                    </button>
                </form>
            @else
                <a href="{{ route('noticias.show', $noticia->id) }}"
                   class="btn btn-outline-info btn-sm" target="_blank">
                    <i class="bi bi-eye me-1"></i>Ver noticia pública
                </a>
            @endif
        </div>

        <div class="table-panel p-4">
            <form method="POST"
                  action="{{ route('dashboard.noticias.update', $noticia->id) }}">
                @csrf
                @method('PUT')

                <div class="mb-3">
                    <label class="form-label text-secondary small text-uppercase"
                           style="letter-spacing:1px;">Título *</label>
                    <input type="text" name="titulo"
                           class="form-control bg-dark text-white border-secondary @error('titulo') is-invalid @enderror"
                           value="{{ old('titulo', $noticia->titulo) }}" required>
                    @error('titulo')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>

                <div class="mb-3">
                    <label class="form-label text-secondary small text-uppercase"
                           style="letter-spacing:1px;">Contenido *</label>
                    <textarea name="contenido" rows="12"
                              class="form-control bg-dark text-white border-secondary @error('contenido') is-invalid @enderror"
                              required>{{ old('contenido', $noticia->contenido) }}</textarea>
                    @error('contenido')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>

                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        <label class="form-label text-secondary small text-uppercase"
                               style="letter-spacing:1px;">Evento relacionado</label>
                        <select name="id_evento"
                                class="form-select bg-dark text-white border-secondary @error('id_evento') is-invalid @enderror">
                            <option value="">— Sin evento relacionado —</option>
                            @foreach($eventos as $evento)
                                <option value="{{ $evento->id }}"
                                    {{ old('id_evento', $noticia->id_evento) == $evento->id ? 'selected' : '' }}>
                                    {{ $evento->nombre }} ({{ $evento->fecha->format('d M Y') }})
                                </option>
                            @endforeach
                        </select>
                        @error('id_evento')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label text-secondary small text-uppercase"
                               style="letter-spacing:1px;">Estado *</label>
                        <select name="estado"
                                class="form-select bg-dark text-white border-secondary @error('estado') is-invalid @enderror"
                                required>
                            <option value="borrador"
                                {{ old('estado', $noticia->estado) === 'borrador' ? 'selected' : '' }}>
                                Borrador
                            </option>
                            <option value="publicada"
                                {{ old('estado', $noticia->estado) === 'publicada' ? 'selected' : '' }}>
                                Publicada
                            </option>
                        </select>
                        @error('estado')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                </div>

                {{-- Info de publicación --}}
                @if($noticia->publicado_en)
                <div class="alert alert-secondary small mb-4">
                    <i class="bi bi-calendar me-1"></i>
                    Publicada el {{ $noticia->publicado_en->isoFormat('D [de] MMMM [de] YYYY') }}
                    por {{ $noticia->autor->nombre }}
                </div>
                @endif

                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-warning text-dark">
                        <i class="bi bi-save me-1"></i>Guardar cambios
                    </button>
                    <a href="{{ route('dashboard.noticias.index') }}"
                       class="btn btn-outline-secondary">Cancelar</a>
                </div>

            </form>
        </div>
    </div>
</div>
@endsection